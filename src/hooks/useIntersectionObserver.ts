...
/**
 * IntersectionObserver를 사용하여, 대상 요소가 뷰포트와 교차하는지 감지하는 훅
...
const useIntersectionObserver = ({ target, onIntersect, enabled, rootMargin = '0px' }: Props) => {
  useEffect(() => {
    if (!enabled) return;
  ...
  }, [enabled, onIntersect, target, rootMargin]);
};
